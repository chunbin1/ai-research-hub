...
| `09696.HK` / `01772.HK` | `{ '9696.HK', 'HK' }` | 数字补/截到 **4 位**,统一 `.HK` |
| `0700.HK` / `700.HK` | `{ '0700.HK', 'HK' }` | 腾讯。Yahoo 只认 4 位,`700.HK` 是 404 |
...
**注意:120 根预热门槛不在这里。** 「数据不足就不给信号」是业务策略,不是数学规则,
放在 `scanner`(Task 9)里。本函数只做数学上必要的守卫:`bars.length < period + 1`
时无法算出 ATR,返回 `[]`。
...
- 从下标 `period-1` 开始输出;`period < 2` 或 `bars.length < period + 1` 时返回 `[]`
  (`period` 为 1 时循环首轮就要读 `bars[-1]`,会抛 TypeError)
...
注意:即使根数仍是 1254,**每根的价格也可能有 ~1e-7 的浮点差异** —— Yahoo 的
adjclose 在两次请求间会抖动(实测同一天两次取数,1254 根里 747 根不同)。
Step 3 的期望值是按本次生成的 fixture 算的;如果你重新生成了 fixture 并且断言失败,
那不是引擎的问题,而是输入变了。
...
2. **丢弃未收盘的 bar**:判据是「**当前正处在交易时段内,且最后一根就是这个时段那天**」——
   `regular.start <= now < regular.end` 且 `最后一根日期 === utcDate(regular.start)`。
   **不要用 `meta.regularMarketTime`**:它指的是最近一次成交的时刻(收盘后即上一场的
   收盘时间),而 `currentTradingPeriod` 指的是今天这一场(开盘前描述的是还没开始的那场)。
   开盘前两者分属不同日期,混着比会把**已经收盘**的那根误删 —— 实测 2026-08-21 08:53 UTC
   (美股 13:30 才开盘)会丢掉已收盘的 08-20。
3. **跳过 null**:`close` / `adjclose` / `high` / `low` **任缺其一**就整根跳过。
   只挡 `close` 不够 —— JS 里 `null` 参与算术会静默变成 `0`,`high`/`low` 为 `null`
   会让整根价格塌成 0 一路流进引擎。边界处还要校验这三个字段确实是数组,
   否则结构畸形的响应会抛裸 `TypeError` 而不是 `YahooError`,调用方按 kind 分流的
   容错逻辑就会失效、单只票的问题升级成整批扫描崩溃。
...
test('high 或 low 单独为 null 的一根也跳过,不能塌成 0', async () => {
  ...
  const payload = fakePayload({ closes: [100, 101, 102], session: sessionOn(2) })
  payload.chart.result[0].indicators.quote[0].high[1] = null
  const s = await fetchDailyQuotes('ALB', {
    fetchImpl: okFetch(payload) as unknown as typeof fetch,
    nowMs: () => (sessionOn(2).end + 3600) * 1000,
  })
...
- Consumes: `listScannable` / `updateScanResult` from `../watchlistStore.js`;
  ...
  `supertrend` from `./supertrend.js`;`toWeekly` from `./weekly.js`;
  `getSetting` from `../siteSettingsStore.js`
- Produces:
  - `const MIN_BARS = 120` —— 预热门槛,日线与周线**各自**判断
...
**标题从哪来:** 直接用正则扫原文的 ATX 标题行,按出现顺序去重。**正文一概不看。**

⚠️ **不要走 `parseMarkdown` 的 `chunks[].section_title`** —— 曾经这么写过,是个真 bug:
chunks 只为**有正文内容**的小节产出,而研报里主标的所在的那个标题(如
`### 4.0 腾讯控股（0700.HK / TCEHY）`)下面往往紧跟着子标题、自己没有正文段落,
于是整条标题连同代码一起消失。线上七篇研报实测:走 chunks 抽到 8 只,直接扫标题
抽到 13 只,**漏掉的 5 只全是各篇的主标的**(0700.HK 腾讯、0883.HK 中海油、
1088.HK 中国神华),其中两篇研报因此抽出 0 只、看起来像「没提到任何标的」。
...
Run: `cd packages/server && pnpm test 2>&1 | tail -20`
Expected: PASS,5 个新 test 全绿(其中阈值那条现在覆盖两侧)
...
| GET | `/signals/:symbol/log` | 公开 | 单票日志,`?timeframe=1d&limit=60` |
...
// 而且要在 auth.ts 被求值之前设好 —— 它在模块顶层读一次 env 存成常量。
...
// 这里直接对它建的表写一行,凑出 seedALB() 引用的 doc_1。
...
async function freshApp() {
  const db = new Database(':memory:')
  // rowOf() 会调 getDocument() 回填来源报告,documentStore 不初始化就会抛
...
创建 `packages/server/src/routes/signals.disabled.test.ts`:
...
- Modify: `packages/client/src/pages/SignalsPage.test.tsx`(见 Step 6 末尾)
...
**同时把看板 Table 的 `scroll` 从 `{ x: 'max-content' }` 改成确定的像素宽度:**
...
这个塌陷 **jsdom 测不出来**(不计算布局),只能在浏览器里量 —— 验收时展开任意一行,
确认 `document.querySelector('.ant-table > .ant-table-container table').scrollWidth`
仍是四位数而不是 500048。

**同时要改 `packages/client/src/pages/SignalsPage.test.tsx` 的 fetch stub。**
它原本对所有 URL 一律返回 `{ rows }`,但看板页现在还会请求 `/events`(横幅)
和 `/log`(展开行) —— 那两个组件会拿到 `undefined`。把 `stubRows` 改成按 URL 分流:
