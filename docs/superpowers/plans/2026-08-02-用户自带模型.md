...
本计划已于 2026-08-02 在分支 `feat/byok` 上执行完毕。执行过程中的 code review 发现**下面三处代码块里的错误是本计划自己写错的**,已在实现时修正。若要复用本计划,以这一节为准 —— 下方对应任务里的原始代码块保留原样,作为发生了什么的记录。

完整执行记录见 `.superpowers/sdd/progress.md`。

### 1. Task 2 —— `isPrivateAddress` 有可复现的 SSRF 绕过

原代码用正则 `^::ffff:(\d+\.\d+\.\d+\.\d+)$` 识别 IPv4-mapped IPv6。但 WHATWG `URL` 会把括号字面量序列化成**十六进制组**:

```
new URL('https://[::ffff:127.0.0.1]/v1').hostname === '[::ffff:7f00:1]'
```

正则不匹配,落到 `head` 检查时 `lower.split(':')[0]` 是空串 → `head = 0` → **判为公网放行**。实测 `[::ffff:127.0.0.1]`、`[::ffff:10.0.0.1]`、`[::ffff:169.254.169.254]`(云元数据端点)全部能穿过 `assertPublicBaseURL`。

正确做法:不要在正则上打补丁,把地址展开成 8 个 hextet 组(处理 `::` 压缩与内嵌点分 IPv4),若组 0–4 全零且组 5 为 `0xffff`,就从组 6/7 还原出 IPv4 交给 `isPrivateIPv4`;`fc00::/7` 与 `fe80::/10` 也改成对解析后的数值组做掩码,而不是对字符串切片。顺带修掉完全展开形式 `0:0:0:0:0:0:0:1` 的同类问题。

**测试也要改**:原测试只用已经点分好的字符串直接调 `isPrivateAddress`,照不出这个洞。必须有走完整 `assertPublicBaseURL(括号字面量)` 的用例,外加一个公网 IPv6 正对照证明没有过度拦截。

### 2. Task 7 —— 「使用我自己的模型」开关会静默毁掉已保存的 key
...
而 `onSave` 会连同**当前表单里的所有值**一起提交,包括 `apiKey`。于是:用户已存了一把能用的 key → 开始输入新 key(或改了模型)但还没保存 → 顺手点开关暂停 BYOK → 那半截没验证过的输入覆盖掉了原来能用的配置 → 随后 `setApiKey('')` 把证据也清了。开关还不受 `canSubmit` / `saving` 约束。

正确做法:给开关独立的 handler,只提交 `data.config` 里**已保存**的值、**无条件省略 `apiKey`**、不碰用户草稿,并加 `disabled={saving}`:
...
仍然只用 4 个接口 —— `PUT` 省略 `apiKey` 就是改 `enabled` 的受支持方式,不需要第 5 个端点。保存按钮的 `onSave` 保持不变。

顺带:原计划里「save 失败不清空已有配置」那条测试是**空断言** —— 它只断言 `data` 非 null,但挂载 payload 的 `config` 本来就是 `null`,而 hook 的失败分支压根不调 `setData`,所以恒真。要改成挂载时返回一份**有配置**的 payload,再对失败前后的 `data` 做深比较。

### 3. Task 8 —— `outOfQuota` 锁死了自带 key 的用户

`ChatPanel.tsx` 里这行是 BYOK 之前就有的代码,BYOK 上线后就错了:

```tsx
const outOfQuota = user && !user.unlimited && (user.remaining ?? 0) <= 0
```

公共额度用完(`remaining: 0`)的用户去配了自己的 key 之后,服务端已经不再计次,前端却仍然禁用输入框、显示「次数已用完」。**这恰好锁死了这个功能存在的意义所指的那批人。**

正确做法:

```tsx
// 自带 key 花的是用户自己的额度,不受公共计数约束 —— 少了 source 判断会把
// 「用完免费额度才来配 key」的用户锁在门外,而这正是本功能要服务的人。
const outOfQuota = user && !user.unlimited && effective?.source === 'server' && (user.remaining ?? 0) <= 0
```

加载窗口内 `effective` 短暂为 `null`、条件为假、输入框可用,这是**正确的**取舍:偏向"可用"最多多发一次被服务端 403 掉的请求,偏向"禁用"就是上面这个 bug。

这条逻辑要有回归测试(`ChatPanel.test.tsx`),两个方向都测。
...
在 `packages/server/src/types.ts` 顶部,**删掉 `export type LLMProvider = 'anthropic' | 'zhipu'`** —— 全项目只有被重写的 `llm.ts` 引用它,重构后它就是死代码。然后追加:
...
> 这个文件不需要 `serverLLMConfig` —— `GET` 走的是 `resolveLLMConfig`,无配置时它内部就会回落到站长默认。
