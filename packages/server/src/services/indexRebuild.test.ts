...
test('首次重建:写新代、翻转 active_gen、FTS 可查', () => {
  const db = freshDb()
  const v = fakeVectors()
  return rebuildDoc(db, 'doc_a', v.deps).then(o => {
...
test('已经是目标代次的直接跳过,不再写向量', async () => {
  const db = freshDb()
  const v = fakeVectors()
  await rebuildDoc(db, 'doc_a', v.deps)
  const writesAfterFirst = v.calls.writes

  const second = await rebuildDoc(db, 'doc_a', v.deps)
...
test('原文变了则重建,并回收旧代向量', async () => {
  ...
  const v = fakeVectors()
  await rebuildDoc(db, 'doc_a', v.deps)
  const oldGen = getIndexState(db, 'doc_a')!.active_gen

  v.deps.readMarkdown = () => MD_LONGER
  const o = await rebuildDoc(db, 'doc_a', v.deps)
  assert.equal(o.status, 'rebuilt')
...
test('写向量失败时整篇停在旧代,FTS 不动', async () => {
  ...
  const v = fakeVectors()
  await rebuildDoc(db, 'doc_a', v.deps)
  const before = getIndexState(db, 'doc_a')!
  ...
  const after = getIndexState(db, 'doc_a')!
  assert.deepEqual(
...
test('向量只写进去一半时判失败,不翻转', async () => {
  ...
  assert.equal(getIndexState(db, 'doc_a')!.active_gen, LEGACY_GEN, '没验证通过就不该翻转')
})
...
test('失败后修好原因重跑,这次成功', async () => {
  ...
  const v = fakeVectors()
  v.writePartial(1)
  assert.equal((await rebuildDoc(db, 'doc_a', v.deps)).status, 'failed')

  v.writeAll()
  const o = await rebuildDoc(db, 'doc_a', v.deps)
  assert.equal(o.status, 'rebuilt')
  assert.equal(getIndexState(db, 'doc_a')!.active_gen, genOf(MD))
})
...
test('withVectors=false 只换 FTS,vec_gen 保持落后', async () => {
  const db = freshDb()
  const v = fakeVectors()
  const o = await rebuildDoc(db, 'doc_a', v.deps, { withVectors: false })
...
test('换 embedding 模型会让已完成的文档重新建', async () => {
  ...
  const o = await rebuildDoc(db, 'doc_a', { ...v.deps, embedModel: 'bge-m3' })
  assert.equal(o.status, 'rebuilt', '维度都变了,存量向量必须作废重建')
  assert.equal(getIndexState(db, 'doc_a')!.embed_model, 'bge-m3')
})
...
test('状态行已存在时不会重复盖 legacy', async () => {
  ...
  const v = fakeVectors()
  putIndexState(db, {
    doc_id: 'doc_a', active_gen: 'g0', vec_gen: 'g0', fts_gen: 'g0', embed_model: 'embedding-3',
  })
  await rebuildDoc(db, 'doc_a', v.deps)
